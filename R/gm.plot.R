...
function (model, significant = TRUE, data.analysis = 0) 
{
    ...
    for (rrr in 1:row.grid) for (ccc in 1:col.grid) {
        ...
        if (any(m == ":")) {
            ...
            elements = c(m[1])
            for (i in 2:length(m)) if (length(which(elements == 
                m[i])) == 0 && length(which(c(letters, LETTERS) == 
                m[i])) > 0) 
        ...
        if (is.list(data.analysis) || is.matrix(data.analysis)) {
            ...
            else data.ana.tmp = data.analysis
        }
        for (i in 1:length(m)) {
        ...
        for (i in 1:dim(dep_table)[1]) {
            x1 = cos(i/circle_factor)/10 * sqrt(dim(dep_table)[1]) + 
                0.5
            y1 = sin(i/circle_factor)/10 * sqrt(dim(dep_table)[1]) + 
                0.5
        ...
        for (i in 1:(dim(dep_table)[1])) for (j in (1:dim(dep_table)[2])[-i]) {
            ...
            x <- c(cos(i/circle_factor)/10 * sqrt(dim(dep_table)[1]) + 
                0.5 + result[1], cos(j/circle_factor)/10 * sqrt(dim(dep_table)[1]) + 
                0.5 - result[1])
            y <- c(sin(i/circle_factor)/10 * sqrt(dim(dep_table)[1]) + 
                0.5 - result[2], sin(j/circle_factor)/10 * sqrt(dim(dep_table)[1]) + 
                0.5 + result[2])
            if (is.list(data.analysis) || is.matrix(data.analysis)) {
                grid.lines(x = x, y = y, arrow = arrow(length = unit(0.005, 
                  "cm"), type = "open"), gp = gpar(fill = "white", 
                  col = "black", lty = l.type, lwd = abs(lwd * 
                    data.ana.tmp[i, j])))
                if (dep_table[i, j] != 0 || (!significant && 
                  j > i)) 
                  if (x[1] < x[2]) 
            ...
            else grid.lines(x = x, y = y, arrow = arrow(length = unit(0.005, 
                "cm"), type = "open"), gp = gpar(fill = "white", 
                col = "black", lty = l.type, lwd = lwd/5))
        }
        popViewport(2)
        index = index + 1
