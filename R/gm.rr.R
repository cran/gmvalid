    ...
    "biggest", "first"), conf.level = 0.95) 
{
    ...
    if (all(conditions == 0)) 
        for (i in (1:dim(data)[1])[-reference[1]]) for (j in 1:dim(data)[2]) {
            ...
            if (any(res_table > 0)) {
                rr = riskratio.wald(res_table, conf.level = conf.level)
                result[res.index, ] = c(rr$measure[2, ], rr$p.value[2, 
                  1])
                if (j == reference[2]) 
    ...
    if (all(conditions == 0)) 
        nnd.name.2 = paste(c("Marginal Risk Ratio with ", conf.level * 
            100, "% C.I."), collapse = "")
    else nnd.name.2 = paste(c("Stratified Risk Ratio with ", 
